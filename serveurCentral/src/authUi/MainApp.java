        ...
        if (SessionManager.hasSession()) {
            ...
            auth.reconnect(savedPhone, new AuthService.AuthCallback() {
                ...
                public void onSuccess(int userId, String phone, String username, boolean isNewUser) {
                    // Create private socket for reconnection
                    SocketManager userSocket = new SocketManager();
                    try {
                        userSocket.initAuth(network.getSocket(), userId, phone);
                    ...
                    Platform.runLater(() -> {
                        new ChatView(userId, phone, username, network, userSocket).start(new Stage());
                    });
                }
