...
public class MessageService {
    ...
    public void process(Message m, String receiverPhone, byte[] data) {
        ...
        if (receivers != null && !receivers.isEmpty()) {
            ...
            try {
                ...
                model.User senderUser = userDao.searchByPhone(m.getSenderPhone());
                if (senderUser != null && senderUser.getUsername() != null) {
                    senderInfo += "~" + senderUser.getUsername();
                }

                receiver.send(typeToSend,
                        senderInfo,
                        filenameToSend,
                        toSend);
                System.out.println("[MessageService] Message livré à une session de id="
                        + m.getReceiverId());
            } catch (IOException e) {
                System.err.println("[MessageService] Erreur livraison à une session : "
                        + e.getMessage());
            }
        }
        if (persisted && receivers != null && !receivers.isEmpty()) {
    ...
    public void deliverOfflineMessages(int userId, String userPhone,
        ...
        for (Message m : pending) {
            try {
                ...
                handler.send(typeToSend, m.getSenderPhone(),
                        filenameToSend, data);
    ...
    private java.util.List<ClientHandler> findOnlineByPhone(String phone) {
        ...
        for (java.util.List<ClientHandler> handlers : ChatServer.clients.values()) {
            ...
            String connectedPhone = normalizePhone(handlers.get(0).getUserPhone());
        ...
        if (groupId != -1) {
            ...
            for (int memberId : members) {
                ...
                if (receiverClients != null) {
                    for (ClientHandler receiverClient : receiverClients) {
                        try {
            ...
            if (receiverClients != null) {
                for (ClientHandler receiverClient : receiverClients) {
                    try {
                        ...
                        receiverClient.send(typeToSend, "GROUP_" + groupId + ":" + senderInfo, filenameToSend, data);
                    } catch (Exception e) {
